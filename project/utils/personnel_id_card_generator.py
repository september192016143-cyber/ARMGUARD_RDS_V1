...
def _build_front(personnel) -> Image.Image:
    ...
    # Distribute all 3 lines evenly across the full pill zone (y 667..806)
    ZONE_TOP, ZONE_BOT = 667, 806
    slot_h  = (ZONE_BOT - ZONE_TOP) // 3
    offset  = (slot_h - line_h) // 2   # centre text within each slot

    name_y = ZONE_TOP + slot_h * 0 + offset
    id_y   = ZONE_TOP + slot_h * 1 + offset
    date_y = ZONE_TOP + slot_h * 2 + offset
