...
(function () {
  ...
  function render(data) {
    ...
    if (disk.used_pct !== undefined) {
      ...
      document.getElementById('storage-disk-label').textContent =
        pct + '% used';
      const bar = document.getElementById('storage-disk-bar');
      bar.style.width = pct + '%';
      bar.style.background = pctColor(pct);
      document.getElementById('storage-disk-used').textContent =
        'Used: ' + (disk.used || '—');
      document.getElementById('storage-disk-free').textContent =
